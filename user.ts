interface Facility {
    ...
    location: string;
    isOnline: boolean;
    icon: string; // Nombre del Material Icon
}
...
function createFacilityCard(facility: Facility): HTMLElement {
...
function renderFacilities(facilitiesToRender: Facility[]): void {
    const facilityListContainer = document.getElementById('facilityList');
    if (!facilityListContainer) return;
...
const facilityDetailsModal = document.getElementById('facilityDetailsModal') as HTMLElement;
const detailsCloseBtn = facilityDetailsModal?.querySelector('.details-close-btn') as HTMLButtonElement;
const modalFacilityName = document.getElementById('modalFacilityName') as HTMLElement;
const modalFacilityType = document.getElementById('modalFacilityType') as HTMLElement;
const modalFacilityDescription = document.getElementById('modalFacilityDescription') as HTMLElement;
const modalFacilityLocation = document.getElementById('modalFacilityLocation') as HTMLElement;
const modalFacilityStatusText = document.getElementById('modalFacilityStatusText') as HTMLElement;
const modalFacilityStatusIndicator = document.getElementById('modalFacilityStatusIndicator') as HTMLElement;
const modalRegisterBtn = document.getElementById('modalRegisterBtn') as HTMLButtonElement;


function openFacilityDetailsModal(facility: Facility): void {
    if (!facilityDetailsModal) return;
...
detailsCloseBtn?.addEventListener('click', closeFacilityDetailsModal);

facilityDetailsModal?.addEventListener('click', (event) => {
...
const registrationModal = document.getElementById('registrationModal') as HTMLElement;
const registrationCloseBtn = registrationModal?.querySelector('.registration-close-btn') as HTMLButtonElement;
const regFacilityName = document.getElementById('regFacilityName') as HTMLElement;
const regFacilityType = document.getElementById('regFacilityType') as HTMLElement;
const regFacilityDescription = document.getElementById('regFacilityDescription') as HTMLElement;
const registrationForm = document.getElementById('registrationForm') as HTMLFormElement;
const cancelRegBtn = registrationModal?.querySelector('.cancel-reg-btn') as HTMLButtonElement;
const confirmRegBtn = registrationModal?.querySelector('.confirm-reg-btn') as HTMLButtonElement;


let currentFacilityForRegistration: Facility | null = null;

function openRegistrationModal(facility: Facility): void {
    ...
    const regDateInput = document.getElementById('regDate') as HTMLInputElement;
    const regTimeInput = document.getElementById('regTime') as HTMLInputElement;
...
function closeRegistrationModal(): void {
    if (!registrationModal) return;
...
registrationForm?.addEventListener('submit', (event) => {
    ...
    if (currentFacilityForRegistration) {
        const regDate = (document.getElementById('regDate') as HTMLInputElement).value;
        const regTime = (document.getElementById('regTime') as HTMLInputElement).value;
...
function addCardEventListeners(): void {
    document.querySelectorAll('.view-details-btn').forEach(button => {
        button.addEventListener('click', (event) => {
            const facilityId = (event.target as HTMLElement).dataset.facilityId;
...
function handleModalRegisterClick(event: Event): void {
    const facilityId = (event.target as HTMLElement).dataset.facilityId;
    const facility = allFacilities.find(f => f.id === facilityId);
    if (facility) {
        openRegistrationModal(facility);
